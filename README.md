...
企业 B 端 CRM 客户管理系统，采用前后端分离、模块化单体架构。本仓库当前处于第一阶段：已完成工程骨架、分层边界、核心模块、数据库模型和 API 契约设计，尚未实现业务功能。
...
优先实现公共基座（统一响应/异常、审计字段、MyBatis-Plus、Security/JWT、日志与 traceId），再实现 RBAC 和客户主数据，最后按线索转化、商机、合同、统计的依赖顺序迭代。
